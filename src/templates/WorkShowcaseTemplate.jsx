...
/**
 * WorkShowcaseTemplate (updated)
 * - uses horizontal scrolling filter bar
 * - uses CreativeMediaItem (tilt + overlay)
 * - keeps Featured media block
 */

function FilterBar({ tags = ["All"], active, onChange }) {
  return (
    <nav className="h-scroll no-border" aria-label="work filters">
      {tags.map((t) => {
        const isActive = String(t).toLowerCase() === String(active).toLowerCase();
        return (
          <button key={t} type="button" onClick={() => onChange(t)} className={`glass-chip no-border ${isActive ? "active" : ""}`} aria-pressed={isActive}>
            {t}
          </button>
        );
      })}
...
export default function WorkShowcaseTemplate({
  ...
  const filtered = useMemo(() => {
    if (!activeTag || activeTag === "All") return rest;
    return rest.filter((s) => String(s.category).toLowerCase() === String(activeTag).toLowerCase());
  }, [rest, activeTag]);
    ...
    <section className="work-showcase bg-black text-white py-12 md:py-20 px-6 md:px-12 lg:px-20">
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start mb-10">
          <motion.header className="lg:col-span-5" variants={fadeInUp} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.2 }}>
            <h1 className="text-4xl md:text-6xl font-semibold leading-tight mb-4">{title}</h1>
          ...
          <div className="lg:col-span-7">
            <motion.div id="featured" variants={fadeInUp} initial="hidden" whileInView="visible" className="rounded-2xl overflow-hidden border border-transparent">
              {featured && (
                <div className="relative rounded-2xl">
                  {featured.type === "video" ? (
                    <video src={featured.src} poster={featured.poster} muted playsInline loop className="w-full h-64 sm:h-80 lg:h-96 object-cover rounded-2xl" autoPlay />
                  ) : (
                    <img src={featured.src} alt={featured.title} className="w-full h-64 sm:h-80 lg:h-96 object-cover rounded-2xl" loading="lazy" />
                  )}
                  <div className="absolute inset-0 bg-gradient-to-t from-black/40 to-transparent rounded-2xl" />

                  <div className="absolute left-8 bottom-8 text-white">
                    <div className="text-xs uppercase tracking-wider text-neutral-300 mb-2">Featured</div>
                    <div className="text-2xl md:text-3xl font-semibold leading-tight">{featured.title}</div>
                    <div className="mt-2 text-sm text-neutral-300">{featured.category} · {featured.meta?.views || "—"} views · {featured.meta?.duration || "—"}</div>
                  </div>
                </div>
              )}
            </motion.div>
          ...
          <div className="max-w-6xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="rounded-lg p-4 bg-neutral-900/40">
                ...
                <p className="text-neutral-300 mt-3">"The team rebuilt our thumbnails and edits — our CTR jumped 42% in two weeks."</p>
              ...
              <div className="rounded-lg p-4 bg-neutral-900/40">
                ...
                <p className="text-neutral-300 mt-3">"From 3 to 14 minute watch times — their narrative structure works."</p>
              ...
              <div className="rounded-lg p-4 bg-neutral-900/40">
                ...
                <p className="text-neutral-300 mt-3">"Short-form promos drove rapid subscriber growth for our product launch."</p>
        ...
        <div className="mt-12 rounded-2xl p-6 bg-neutral-900/30 flex items-center justify-between">
          ...
          <a href="#contact" className="px-4 py-2 rounded-md btn-primary">Let's Collaborate</a>
        </div>
      </div>
    </section>
  );
